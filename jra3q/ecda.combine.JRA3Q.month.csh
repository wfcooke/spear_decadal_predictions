...
if ( ! -e $yyyy$mm$dd{_}$ss{Z_jra3q.nc} ) then
  ...
  ncap2 -O -v -s 'T=temp.reverse($lev)' ${base_dir}/$yyyy$mm/tmp-hyb-an-gauss.$yyyy$mm.$ll.nc foo_${mm}${ll}.nc  || exit -1
  ncap2 -A -v -s 'U=uwnd.reverse($lev)' ${base_dir}/$yyyy$mm/ugrd-hyb-an-gauss.$yyyy$mm.$ll.nc foo_${mm}${ll}.nc || exit -1
  ncap2 -A -v -s 'V=vwnd.reverse($lev)' ${base_dir}/$yyyy$mm/vgrd-hyb-an-gauss.$yyyy$mm.$ll.nc foo_${mm}${ll}.nc || exit -1
  ncap2 -A -v -s 'Q=spfh.reverse($lev)' ${base_dir}/$yyyy$mm/spfh-hyb-an-gauss.$yyyy$mm.$ll.nc foo_${mm}${ll}.nc || exit -1
  ncks -A -v PS ${base_dir}/$yyyy$mm/PS.$yyyy$mm.$ll.nc foo_${mm}${ll}.nc || exit -1
  ncks -A -C -v PHIS ${script_dir}/data/gpfile_jra3q.nc foo_${mm}${ll}.nc || exit -1
  ncks -A -v HYAI,HYBI ${script_dir}/data/akbk_jra3q.nc foo_${mm}${ll}.nc || exit -1
  mv foo_${mm}${ll}.nc $yyyy$mm$dd{_}$ss{Z_jra3q.nc}
endif 
  @ ll = $ll + 1 ;  set ss = 06
  echo $ll $ss
if ( ! -e $yyyy$mm$dd{_}$ss{Z_jra3q.nc} ) then
