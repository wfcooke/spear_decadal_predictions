...
set daily_files = `find  -name \*.atmos_daily.\* | sed -e "s/^\.\///"`
if ($#daily_files == 6) then
  foreach INFILE ( `ls *.atmos_daily\.*` )
    set TMPFILE = $INFILE:t:s/.atmos_daily./.atmos_month_tmp./
    set OUTFILE = $refineDiagDir/$INFILE:t:s/.atmos_daily./.atmos_month_refined./
    if (-e $INFILE) then
