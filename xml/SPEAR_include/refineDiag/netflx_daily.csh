...
set varlist=(`ls -1 *atmos_daily.*.nc`)
...
foreach sne ( $varlist )
  ...
  if ($status == 0) then
    ...
    ncks -v time_bnds,average_T1,average_T2,average_DT,swup_sfc,swdn_sfc,lwflx,evap,shflx $yr1.atmos_daily.$tilename.nc interim.nc
    # Sum the 5 variables as needed. Output to a atmos_daily_refined file.
    ...
    ncks -x -v swup_sfc,swdn_sfc,lwflx,evap,shflx interim1.nc  $refineDiagDir/$yr1.atmos_daily_refined.$tilename.nc
...
echo "  ---------- end daily analysis ----------  "

echo "  -- end netflx_daily.csh --  "
