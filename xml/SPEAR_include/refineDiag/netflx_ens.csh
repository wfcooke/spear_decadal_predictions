...
set varlist=(`ls -1 *atmos_month.ens_??*.nc`)

foreach sne ( $varlist )
  # Get the ensemble member name
  set ensname=$sne:r:r:e 
  # Get the tile number
  ...
  if ($status == 0) then
    ...
    ncks -v time_bnds,average_T1,average_T2,average_DT,swup_sfc,swdn_sfc,lwflx,evap,shflx $yr1.atmos_month.$ensname.$tilename.nc interim.nc
    ...
    ncks -x -v swup_sfc,swdn_sfc,lwflx,evap,shflx interim1.nc $yr1.atmos_month_refined.$ensname.$tilename.nc
